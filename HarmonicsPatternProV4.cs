...
namespace NinjaTrader.NinjaScript.Indicators
{
    public class ZUPHarmonicsRatioBased : Indicator
    {
        ...
        private DateTime lastPatternDrawTime = DateTime.MinValue;
        ...
        private void DrawHarmonicPattern(HarmonicPattern pattern, bool drawLines = true, bool showLabel = true)
        {
            ...
            bool isNewPattern = pattern.DetectionTime != lastPatternDrawTime;

            // Only clear and redraw if it's a completely new pattern
            if (isNewPattern)
            {
                ClearPatternDrawings(tag, keepRectangle: false);
                
                DateTime endTime = pattern.D.Time.AddDays(3);
                double priceRange = TickSize * 10; // fixed 10 price points around point D
                ...
                lastPatternDrawTime = pattern.DetectionTime;
            }
            else
            {
                // For existing patterns, only clear lines and labels, keep the rectangle
                ClearPatternDrawings(tag, keepRectangle: true);
            }
