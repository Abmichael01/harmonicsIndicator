...
namespace NinjaTrader.NinjaScript.Indicators
{
    public class ZUPHarmonicsRatioBased : Indicator
    {
        ...
        {
            bool isBullish = X.Type == PivotType.High && A.Type == PivotType.Low;
            bool isBearish = X.Type == PivotType.Low && A.Type == PivotType.High;
            ...
            for (int bIndex = aIndex + 1; bIndex < pivots.Count - 2; bIndex++)
            {
                ...
                for (int cIndex = bIndex + 1; cIndex < pivots.Count - 1; cIndex++)
                {
                    ...
                    {
                        var D = pivots[dIndex];
                        if (D.Type != A.Type) continue;
                        double CD = Math.Abs(D.Price - C.Price);
                        double cdRatio = CD / BC;
                        if (!IsWithinRange(cdRatio, ratios.CD_BC)) continue;
                        if (isBullish && !(D.Price < C.Price && D.Price < X.Price)) continue;
                        if (isBearish && !(D.Price > C.Price && D.Price > X.Price)) continue;
