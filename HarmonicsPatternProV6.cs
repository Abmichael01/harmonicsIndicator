...
namespace NinjaTrader.NinjaScript.Indicators
{
    public class ZUPHarmonicsRatioBased : Indicator
    {
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (BarsInProgress == 0)
            {
                ...
                if (latestPattern != null)
                {
                    ...
                    {
                        DrawHarmonicPattern(latestPattern, ShowPatterns);
                    }
                }
            }
        }
        ...
        {
            double tickSize = GetInstrumentTickSize();
            double roundedTarget = Math.Round(targetPrice / tickSize) * tickSize;
            ...
            foreach (var pivot in allPivots)
            {
                ...
                double distance = Math.Abs(pivot.Price - roundedTarget);
                double percentDiff = distance / roundedTarget;
            ...
            if (bestMatch != null)
            {
                double percentDiff = (bestDistance / roundedTarget) * 100;
                Print($"[FIND PIVOT] Found {expectedType} pivot at {bestMatch.Price:F1} within {percentDiff:F2}% of target {roundedTarget:F1}");
            }
        ...
        private void DrawHarmonicPattern(HarmonicPattern pattern, bool drawLines = true)
        {
        ...
        [NinjaScriptProperty]
        [Display(Name = "Show Patterns", Order = 6, GroupName = "Visual")]
